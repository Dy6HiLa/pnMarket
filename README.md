...
Нативная валюта ExcellentEconomy подключается опционально через `provider: excellenteconomy` и `external-id`. Для неё ExcellentEconomy должен быть установлен и зарегистрировать API. Если продавец такой валюты офлайн, покупка безопасно отклоняется, потому что текущий адаптер не выполняет синхронное начисление офлайн-пользователю.
